...
const getAgoraTokenFromDB = async (
  ...
  const streamObj = streamDoc as any

  // Manually set the virtuals since lean() doesn't include them
  const isUpcoming =
    streamObj.streamStatus === 'scheduled' ||
    streamObj.streamStatus === 'starting'
  const isActive =
    streamObj.streamStatus === 'starting' || streamObj.streamStatus === 'live'
...
const getLiveStreamByEventIdFromDB = async (
  eventId: string,
  user?: JwtPayload,
): Promise<ILiveStreamResponseDTO> => {
  const stream = await LiveStream.findOne({ event: eventId })
    .populate('event', 'title description')
    .populate('streamer', 'name email profile')
    .lean()
